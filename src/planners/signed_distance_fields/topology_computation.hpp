...
static int64_t MarkConnectedComponent(
...
template<typename T, typename BackingStore=std::vector<T>>
static uint32_t ComputeConnectedComponents(
        const VoxelGrid::VoxelGrid<T, BackingStore>& source_grid,
        const std::function<bool(const GRID_INDEX&,
                                 const GRID_INDEX&)>& are_connected_fn,
...
static int32_t ComputeConnectivityOfSurfaceVertices(
...
template<typename T, typename BackingStore=std::vector<T>>
static std::map<uint32_t, std::unordered_map<GRID_INDEX, uint8_t>>
...
static std::pair<int32_t, int32_t> ComputeHolesInSurface(
...
template<typename T, typename BackingStore=std::vector<T>>
static std::map<uint32_t, std::pair<int32_t, int32_t>>
...
static std::vector<GRID_INDEX> ExtractStaticSurface(
...
static std::unordered_map<GRID_INDEX, uint8_t> ConvertToDynamicSurface(
...
static std::unordered_map<GRID_INDEX, size_t> BuildSurfaceIndexMap(
        const std::vector<GRID_INDEX>& static_surface)
{
