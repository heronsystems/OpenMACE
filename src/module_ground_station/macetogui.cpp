...
void MACEtoGUI::sendCurrentMissionItem(const int &vehicleID, const std::shared_ptr<MissionTopic::MissionItemCurrentTopic> &component) {
    ...
    QJsonObject json;
    json["message_type"] = "CurrentMissionItem";
    json["agentID"] = static_cast<int>(component->getMissionKey().m_systemID);
...
void MACEtoGUI::sendVehicleTarget(const int &vehicleID, const Abstract_GeodeticPosition* targetPosition) {
    QJsonObject json;
    json["message_type"] = "CurrentVehicleTarget";
...
{
    QJsonObject json;
    json["message_type"] = "GlobalOrigin";
    json["agentID"] = 0;
