...
bool State_LandingDescent::handleCommand(const std::shared_ptr<command_item::AbstractCommandItem> command)
{
    bool success = false;
    clearCommand();
...
void State_LandingDescent::OnEnter(const std::shared_ptr<command_item::AbstractCommandItem> command)
{
    if(command != nullptr)
    {
