...
void Controller_GoTo::FillCommand(const command_item::Action_ExecuteSpatialItem &commandItem, mace_command_goto_t &cmd) const
{
    //commandItem.getSpatialCommand()->setGoToCommand(cmd);
}

void Controller_GoTo::BuildCommand(const mace_command_goto_t &message, command_item::Action_ExecuteSpatialItem &data) const
{
