...
TARGET = module_ml_station
TEMPLATE = lib

DEFINES += MODULE_ML_STATION_LIBRARY
...
SOURCES += module_ml_station.cpp \
            mlguitomace.cpp \
            mlmacetogui.cpp \
            mltimer.cpp

HEADERS += module_ml_station.h \
    mlguitomace.h \
    mlmacetogui.h \
    mlmessagetypes.h \
    mltimer.h \
    module_ml_station_global.h
...
win32:CONFIG(release, debug|release):       lib.files   += release/module_ml_station.lib release/module_ml_station.dll
else:win32:CONFIG(debug, debug|release):    lib.files   += debug/module_ml_station.lib debug/module_ml_station.dll
INSTALLS += lib
