...
private:
    ...
    {
        static_cast<State_FlightGuided_GeoTarget*>(p)->constructAndSendTarget(target);
    }
    ...
    void constructAndSendTarget(const command_target::DynamicTarget &command)
    {
        ...
        tgt.target = command;
        static_cast<MAVLINKVehicleControllers::ControllerGuidedTargetItem_Global*>(Owner().ControllersCollection()->At("GeodeticTargetController"))->Broadcast(tgt, sender);
    }
