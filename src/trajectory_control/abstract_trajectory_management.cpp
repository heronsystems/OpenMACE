...
TrajectoryManagement_Base::TrajectoryManagement_Base(const std::string &agentParamsINIPath, const std::string &environmentINIPath, const std::string &scenarioINIPath)
{
    INIBase::populateAgentParamsFromINI(agentParamsINIPath, m_AgentParams);
}

TrajectoryManagement_Base::TrajectoryManagement_Base(const AgentParams &params) : m_AgentParams(params)
{
}
...
void TrajectoryManagement_Base::updateAgentParams(const AgentParams &params)
{
...
std::vector<pose::CartesianPosition_3D> TrajectoryManagement_Base::retrieveCurrentHorizon() const
{
    ...
    for(size_t i = 0; i < horizonTrajectory.size(); i++)
    {
        ...
        vertex.updateFromDataVector(horizonTrajectory.at(i)._position);
...
{
    _mutex_TargetUpdate.lock();
    if (m_CartesianTargetLambda.find(0) != m_CartesianTargetLambda.cend())
        m_CartesianTargetLambda.erase(0);
    m_CartesianTargetLambda.insert({0, lambda});
    _mutex_TargetUpdate.unlock();
}
