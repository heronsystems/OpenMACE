...
//Modified for Arduplane State Machine Kenneth Kroeger
...
#include "arduplane_state_types.h" //This could be templated out but we don't have the time for that
...
struct State
{
	State()
        ...
        , mStateDebugName(0)
        , currentStateEnum(arduplane::state::ArduplaneFlightState::STATE_GROUNDED)
        , desiredStateEnum(arduplane::state::ArduplaneFlightState::STATE_GROUNDED)
	{
	}
    ...
    arduplane::state::ArduplaneFlightState getCurrentStateEnum() const
    {
        return currentStateEnum;
    }

    arduplane::state::ArduplaneFlightState getDesiredStateEnum() const
    {
...
public:
    void setDesiredStateEnum(const arduplane::state::ArduplaneFlightState &state)
    {
        desiredStateEnum = state;
    }

    void setCurrentStateEnum(const arduplane::state::ArduplaneFlightState &state)
    {
        currentStateEnum = state;
    }
protected:
    arduplane::state::ArduplaneFlightState currentStateEnum;
    arduplane::state::ArduplaneFlightState desiredStateEnum;
