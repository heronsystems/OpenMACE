...
bool AP_State_FlightGuided_CarTarget::handleCommand(const std::shared_ptr<command_item::AbstractCommandItem> command)
{
...
void AP_State_FlightGuided_CarTarget::OnEnter(const std::shared_ptr<command_item::AbstractCommandItem> command)
{
    if(command == nullptr)
    {
        this->OnEnter();
