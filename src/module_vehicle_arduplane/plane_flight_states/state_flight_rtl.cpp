...
hsm::Transition AP_State_FlightRTL::GetTransition()
{
    ...
    if(_currentState != _desiredState)
    {
        ...
        default:
            std::cout<<"I dont know how we ended up in this transition state from State_Flight_RTL."<<std::endl;
            break;
        }
    }
    return rtn;
...
bool AP_State_FlightRTL::handleCommand(const std::shared_ptr<command_item::AbstractCommandItem> command)
{
    UNUSED(command);
    return true;
}
...
void AP_State_FlightRTL::OnEnter(const std::shared_ptr<command_item::AbstractCommandItem> command)
{
    UNUSED(command);
    this->OnEnter();
}
