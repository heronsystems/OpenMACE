...
hsm::Transition AP_State_FlightLoiter::GetTransition()
{
    ...
    if(_currentState != _desiredState)
    {
        ...
        default:
            std::cout<<"I dont know how we ended up in this transition state from State_Flight_Loiter."<<std::endl;
            break;
        }
    }
    return rtn;
...
bool AP_State_FlightLoiter::handleCommand(const std::shared_ptr<command_item::AbstractCommandItem> command)
{
    UNUSED(command);
    return false;
}
...
void AP_State_FlightLoiter::OnEnter(const std::shared_ptr<command_item::AbstractCommandItem> command)
{
    UNUSED(command);
    this->OnEnter();
}
