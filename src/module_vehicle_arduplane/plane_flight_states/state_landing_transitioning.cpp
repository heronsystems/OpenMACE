...
hsm::Transition AP_State_LandingTransitioning::GetTransition()
{
    ...
    if(_currentState != _desiredState)
    {
        ...
        default:
            std::cout<<"I dont know how we ended up in this transition state from State_Landing_Transitioning."<<std::endl;
            break;
        }
    }
    return rtn;
...
bool AP_State_LandingTransitioning::handleCommand(const std::shared_ptr<command_item::AbstractCommandItem> command)
{
    bool success = false;
    clearCommand();
...
void AP_State_LandingTransitioning::OnEnter(const std::shared_ptr<command_item::AbstractCommandItem> command)
{
    this->OnEnter();
    if(command != nullptr)
    {
