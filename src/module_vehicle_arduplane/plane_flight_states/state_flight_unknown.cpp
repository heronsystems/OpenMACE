...
hsm::Transition AP_State_FlightUnknown::GetTransition()
{
    ...
    if(_currentState != _desiredState)
    {
        ...
        default:
            std::cout<<"I dont know how we ended up in this transition state from State_Flight_Unknown."<<std::endl;
            break;
        }
    }
    return rtn;
...
bool AP_State_FlightUnknown::handleCommand(const std::shared_ptr<command_item::AbstractCommandItem> command)
{
    UNUSED(command);
    return false;
}
...
void AP_State_FlightUnknown::OnEnter(const std::shared_ptr<command_item::AbstractCommandItem> command)
{
    UNUSED(command);
    this->OnEnter();
}
