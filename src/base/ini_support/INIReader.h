...
This file has been heavily modified to meet the requirements for ini file parsing 
for APL program. 
                     ...
                     void* user)
{
    ...
    while (reader(line, INI_MAX_LINE, stream) != NULL) {
        ...
        rstrip(start);
            ...
            else if(*end == '\0') //only looking for lines with this
            {
                name = start;
                value = start;
                if (!handler(user, section, name, value) && !error)
...
public:
    ...
    const std::set<std::string> Fields(const std::string &section) const;
...
inline const std::set<std::string> INIReader::Fields(const std::string &section) const
{
