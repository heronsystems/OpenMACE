...
struct CHUNK_REGION
{
    ...
    CHUNK_REGION(const double base_x, const double base_y, const double base_z)
        : base(Eigen::Vector3d(base_x, base_y, base_z)) {}
...
protected:
    ...
    void SafetyCheckSizes(const double chunk_x_size,
                          const double chunk_y_size,
                          const double chunk_z_size) const
    {
        if (chunk_x_size <= 0.0)
        {
    ...
    int64_t GetDataIndex(const int64_t x_index, const int64_t y_index, const int64_t z_index) const
    {
...
public:
    ...
    bool IndexInBounds(const int64_t x_index, const int64_t y_index, const int64_t z_index) const
    {
    ...
    std::pair<const T&, bool> GetImmutableByIndex(const int64_t x_index, const int64_t y_index, const int64_t z_index) const
    {
    ...
    std::pair<T&, bool> GetMutableByIndex(const int64_t x_index, const int64_t y_index, const int64_t z_index)
    {
        assert(chunk_initialized_ || cell_initialized_);
        if (IndexInBounds(x_index, y_index, z_index))
        {
    ...
    bool SetChunkValue(const T& value)
    {
        assert(chunk_initialized_);
        assert(data_.size() == 1);
        data_[0] = value;
    ...
    std::vector<double> GetIndexLocationInGrid(const int64_t x_index, const int64_t y_index, const int64_t z_index) const
    {
        assert(chunk_initialized_ || cell_initialized_);
        if (IndexInBounds(x_index, y_index, z_index))
        {
    ...
    std::vector<double> GetElementSize() const
    {
        assert(chunk_initialized_ || cell_initialized_);
        if (chunk_initialized_)
        {
...
public:
    ...
    void CoreInitialize(const double cell_x_size, const double cell_y_size, const double cell_z_size, const int64_t chunk_num_x_cells, const int64_t chunk_num_y_cells, const int64_t chunk_num_z_cells, const T& default_value)
    {
...
public:
    ...
    void Initialize(const Eigen::Isometry3d& origin_transform, const double cell_x_size, const double cell_y_size, const double cell_z_size, const int64_t chunk_num_x_cells, const int64_t chunk_num_y_cells, const int64_t chunk_num_z_cells, const T& default_value)
    {
        SafetyCheckSizes(cell_x_size, cell_y_size, cell_z_size, chunk_num_x_cells, chunk_num_y_cells, chunk_num_z_cells);
    ...
    void Initialize(const double cell_x_size, const double cell_y_size, const double cell_z_size, const int64_t chunk_num_x_cells, const int64_t chunk_num_y_cells, const int64_t chunk_num_z_cells, const T& default_value)
    {
        SafetyCheckSizes(cell_x_size, cell_y_size, cell_z_size, chunk_num_x_cells, chunk_num_y_cells, chunk_num_z_cells);
        CoreInitialize(cell_x_size, cell_y_size, cell_z_size, chunk_num_x_cells, chunk_num_y_cells, chunk_num_z_cells, default_value);
        origin_transform_.setIdentity();
    ...
    std::pair<const T&, FOUND_STATUS> GetImmutable(const double x, const double y, const double z) const
    {
        Eigen::Vector3d location(x, y, z);
        return GetImmutable(location);
    }

    std::pair<T&, FOUND_STATUS> GetMutable(const double x, const double y, const double z)
    {
        Eigen::Vector3d location(x, y, z);
        return GetMutable(location);
    }

    SET_STATUS SetCellValue(const double x, const double y, const double z, const T& value)
    {
        Eigen::Vector3d location(x, y, z);
        return SetCellValue(location, value);
    }

    SET_STATUS SetCellValue(const double x, const double y, const double z, T&& value)
    {
        Eigen::Vector3d location(x, y, z);
        return SetCellValue(location, value);
    }

    SET_STATUS SetChunkValue(const double x, const double y, const double z, const T& value)
    {
        Eigen::Vector3d location(x, y, z);
        return SetCellValue(location, value);
    }

    SET_STATUS SetChunkValue(const double x, const double y, const double z, T&& value)
    {
        Eigen::Vector3d location(x, y, z);
        return SetCellValue(location, value);
    }

    CHUNK_REGION GetContainingChunkRegion(const Eigen::Vector3d& grid_location) const
    {
        assert(initialized_);
    ...
    std::pair<const T&, FOUND_STATUS> GetImmutable(const Eigen::Vector3d& location) const
    {
        assert(initialized_);
    ...
    std::pair<T&, FOUND_STATUS> GetMutable(const Eigen::Vector3d& location)
    {
        assert(initialized_);
        Eigen::Vector3d grid_location = inverse_origin_transform_ * location;
        CHUNK_REGION region = GetContainingChunkRegion(grid_location);
    ...
    SET_STATUS SetCellValue(const Eigen::Vector3d& location, const T& value)
    {
        assert(initialized_);
        Eigen::Vector3d grid_location = inverse_origin_transform_ * location;
        CHUNK_REGION region = GetContainingChunkRegion(grid_location);
    ...
    SET_STATUS SetCellValue(const Eigen::Vector3d& location, T&& value)
    {
        assert(initialized_);
        Eigen::Vector3d grid_location = inverse_origin_transform_ * location;
        CHUNK_REGION region = GetContainingChunkRegion(grid_location);
    ...
    SET_STATUS SetChunkValue(const Eigen::Vector3d& location, const T& value)
    {
        assert(initialized_);
        Eigen::Vector3d grid_location = inverse_origin_transform_ * location;
        CHUNK_REGION region = GetContainingChunkRegion(grid_location);
    ...
    SET_STATUS SetChunkValue(const Eigen::Vector3d& location, T&& value)
    {
        assert(initialized_);
        Eigen::Vector3d grid_location = inverse_origin_transform_ * location;
        CHUNK_REGION region = GetContainingChunkRegion(grid_location);
    ...
    std::vector<double> GetCellSizes() const
    {
        return std::vector<double>{cell_x_size_, cell_y_size_, cell_z_size_};
    }

    std::vector<double> GetChunkSizes() const
    {
    ...
    std::vector<int64_t> GetChunkNumCells() const
    {
    ...
    const std::unordered_map<CHUNK_REGION, DynamicSpatialHashedVoxelGridChunk<T>>& GetInternalChunks() const
    {
        return chunks_;
    }
