...
#define MACE_MSG_ID_AUCTION_NEW_TASK_KEY_CRC 165
#define MACE_MSG_ID_10005_CRC 165
...
#define MACE_MESSAGE_INFO_AUCTION_NEW_TASK_KEY { \
    ...
    {  { "creatorID", NULL, MACE_TYPE_UINT64_T, 0, 0, offsetof(mace_auction_new_task_key_t, creatorID) }, \
         ...
         { "type", NULL, MACE_TYPE_INT8_T, 0, 20, offsetof(mace_auction_new_task_key_t, type) }, \
         } \
}
#else
#define MACE_MESSAGE_INFO_AUCTION_NEW_TASK_KEY { \
...
static inline uint16_t mace_msg_auction_new_task_key_pack(uint8_t system_id, uint8_t component_id, mace_message_t* msg,
                               uint64_t creatorID, uint32_t taskID, double taskGenTime, int8_t type)
{
...
static inline void mace_msg_auction_new_task_key_send(mace_channel_t chan, uint64_t creatorID, uint32_t taskID, double taskGenTime, int8_t type)
{
#if MACE_NEED_BYTE_SWAP || !MACE_ALIGNED_FIELDS
    char buf[MACE_MSG_ID_AUCTION_NEW_TASK_KEY_LEN];
    _mace_put_uint64_t(buf, 0, creatorID);
 ...
 */
static inline void mace_msg_auction_new_task_key_send_buf(mace_message_t *msgbuf, mace_channel_t chan,  uint64_t creatorID, uint32_t taskID, double taskGenTime, int8_t type)
{
#if MACE_NEED_BYTE_SWAP || !MACE_ALIGNED_FIELDS
 ...
 * @return Task type
 */
static inline int8_t mace_msg_auction_new_task_key_get_type(const mace_message_t* msg)
{
    return _MACE_RETURN_int8_t(msg,  20);
}
