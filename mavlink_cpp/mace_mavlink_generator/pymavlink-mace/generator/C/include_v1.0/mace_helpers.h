#ifndef  _MACE_HELPERS_H_
#define  _MACE_HELPERS_H_
...
#endif
{
	...
	// One sequence number per channel
        msg->seq = mace_get_channel_status(chan)->current_tx_seq;
        mace_get_channel_status(chan)->current_tx_seq = mace_get_channel_status(chan)->current_tx_seq+1;
        msg->checksum = crc_calculate(((const uint8_t*)(msg)) + 3, MACE_CORE_HEADER_LEN);
        crc_accumulate_buffer(&msg->checksum, _MACE_PAYLOAD(msg), msg->len);
#if MACE_CRC_EXTRA
	crc_accumulate(crc_extra, &msg->checksum);
        ...
        return length + MACE_NUM_NON_PAYLOAD_BYTES;
}
...
MACE_HELPER uint16_t mace_finalize_message(mace_message_t* msg, uint8_t system_id, uint8_t component_id,
						 uint8_t length)
{
        return mace_finalize_message_chan(msg, system_id, component_id, MACE_COMM_0, length);
}
#endif
 ...
 */
#if MACE_CRC_EXTRA
MACE_HELPER void _mav_finalize_message_chan_send(mace_channel_t chan, uint8_t msgid, const char *packet,
						    uint8_t min_length, uint8_t length, uint8_t crc_extra)
#else
MACE_HELPER void _mav_finalize_message_chan_send(mace_channel_t chan, uint8_t msgid, const char *packet, uint8_t length)
#endif
{
	uint16_t checksum;
	...
	status->current_tx_seq++;
        checksum = crc_calculate((const uint8_t*)&buf[1], MACE_CORE_HEADER_LEN);
	...
	ck[1] = (uint8_t)(checksum >> 8);
        ...
        MACE_START_UART_SEND(chan, MACE_NUM_NON_PAYLOAD_BYTES + (uint16_t)length);
        _mace_send_uart(chan, (const char *)buf, MACE_NUM_HEADER_BYTES);
        _mace_send_uart(chan, packet, length);
        _mace_send_uart(chan, (const char *)ck, 2);
        MACE_END_UART_SEND(chan, MACE_NUM_NON_PAYLOAD_BYTES + (uint16_t)length);
}
...
MACE_HELPER void _mace_resend_uart(mace_channel_t chan, const mace_message_t *msg)
{
	...
	// XXX use the right sequence here
        ...
        MACE_START_UART_SEND(chan, MACE_NUM_NON_PAYLOAD_BYTES + msg->len);
        _mace_send_uart(chan, (const char *)&msg->magic, MACE_NUM_HEADER_BYTES);
        _mace_send_uart(chan, _MACE_PAYLOAD(msg), msg->len);
        _mace_send_uart(chan, (const char *)ck, 2);
        MACE_END_UART_SEND(chan, MACE_NUM_NON_PAYLOAD_BYTES + msg->len);
}
...
{
        memcpy(buffer, (const uint8_t *)&msg->magic, MACE_NUM_HEADER_BYTES + (uint16_t)msg->len);

        uint8_t *ck = buffer + (MACE_NUM_HEADER_BYTES + (uint16_t)msg->len);

	ck[0] = (uint8_t)(msg->checksum & 0xFF);
	ck[1] = (uint8_t)(msg->checksum >> 8);

        return MACE_NUM_NON_PAYLOAD_BYTES + (uint16_t)msg->len;
}
                                                 ...
                                                 mace_status_t* r_mace_status)
{
	...
	*/
#if MACE_CRC_EXTRA
#ifndef MACE_MESSAGE_CRC
        static const uint8_t mace_message_crcs[256] = MACE_MESSAGE_CRCS;
#define MACE_MESSAGE_CRC(msgid) mace_message_crcs[msgid]
#endif
#endif
...
/* Support shorter buffers than the
   default maximum packet size */
...
#ifdef MACE_CHECK_MESSAGE_LENGTH
                if (rxmsg->len != MACE_MESSAGE_LENGTH(c))
		{
			status->parse_error++;
                        status->parse_state = MACE_PARSE_STATE_IDLE;
			break;
		...
		{
                        status->parse_state = MACE_PARSE_STATE_GOT_MSGID;
		}
		break;

        case MACE_PARSE_STATE_GOT_MSGID:
...
#if MACE_CRC_EXTRA
                mace_update_checksum(rxmsg, MACE_MESSAGE_CRC(rxmsg->msgid));
#endif
        ...
        case MACE_PARSE_STATE_GOT_BAD_CRC1:
			...
			// Successfully got message
                ...
                status->parse_state = MACE_PARSE_STATE_IDLE;
                _MACE_PAYLOAD_NON_CONST(rxmsg)[status->packet_idx+1] = (char)c;
	...
	status->parse_error = 0;
		...
		/*
		  ...
		  checksum
		 */
                r_message->checksum = _MACE_PAYLOAD(rxmsg)[status->packet_idx] | (_MACE_PAYLOAD(rxmsg)[status->packet_idx+1]<<8);
	}
 ...
 */
MACE_HELPER uint8_t put_bitfield_n_by_index(int32_t b, uint8_t bits, uint8_t packet_index, uint8_t bit_index, uint8_t* r_bit_index, uint8_t* buffer)
{
