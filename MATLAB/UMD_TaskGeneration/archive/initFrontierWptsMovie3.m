function plotHandles  = initFrontierWptsMovie3(swarmWorld, swarmState, targetState, trueWorld, runParams, swarmModel, targetModel, plotHandles)
subplot(plotHandles.subplotHandle)

switch swarmModel.communicationTopology
    case 'allToAll'
        cellStateMat = swarmWorld{3}.cellStateMat;
        plotHandles.figh_subplot2 = imagesc(trueWorld.xcp,trueWorld.ycp,cellStateMat,[0 2]);
        title('Frontier (agent 3)')
        set(gca,'YDir','normal')
        % colorbar;
        set(gca,'FontSize',16)
        xlabel('X (m)')
        ...
        hold on;
        % plot frontier nodes
        frontier = [trueWorld.nodeX((swarmWorld{3}.frontierIndex)) trueWorld.nodeY((swarmWorld{3}.frontierIndex))];
        if ~isempty(frontier)
        ...
        if ( strcmp(swarmModel.mapping.method,'frontierAndBlob') )
            plotHandles.figh_sublobCentroid = scatter(swarmWorld{3}.subblobCentroid(:,1),swarmWorld{3}.subblobCentroid(:,2),30,'k','o','linewidth',1,'MarkerFaceColor','flat');
        end
