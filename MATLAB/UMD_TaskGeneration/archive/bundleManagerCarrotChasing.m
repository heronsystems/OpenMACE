function bundleManagerCarrotChasing( ROS_MACE, wptsDesired, captureRadius)
...
while(~all(reaching_wpts_end))
    ...
    if ( ~isempty(msg) )
        ...
        if ~(dist <= captureRadius)
            % if not reaching capture radius, then send the carrot chasing
            % waypoint
            
            % first compute the cc waypoint
            waypointRequest = rosmessage(ROS_MACE.waypointClient);
            waypointRequest.Timestamp = rostime('now');
            waypointRequest.VehicleID = ROS_MACE.agentIDs(agentIndex);   % Vehicle ID
            waypointRequest.CommandID = 3; % TODO: Set command ID enum in MACE
            ...
            if cur_wptIndex(agentIndex) == 1
                carrotDirection = (targetWpt-pos)/norm(targetWpt-pos);
                ccWpt = pos+carrotDirection*ROS_MACE.deltaAhead;
            else
                prevTargetWpt = wptsDesired{agentIndex}(cur_wptIndex(agentIndex)-1,:);
                projection = pointToLineProjection([prevTargetWpt;targetWpt],pos);
                carrotDirection = (targetWpt-projection)/norm(targetWpt-projection);
                
                ccWpt = projection + carrotDirection*ROS_MACE.deltaAhead;
            end
            
            % packup and send the cc wpt
            switch ROS_MACE.coordSys
                case 'ENU'
                    waypointRequest.Easting = ccWpt(1); % Relative easting position to Datum
                    waypointRequest.Northing = ccWpt(2); % Relative northing position to Datum
                case 'F3'
                    [east, north] = F3toENU(ccWpt(1),ccWpt(2));
                    waypointRequest.Easting = east; % Relative easting position to Datum
                    waypointRequest.Northing = north; % Relative northing position to Datum
            end
            waypointRequest.Altitude = ROS_MACE.operationalAlt(agentIndex);
            ...
            % if reached capture radius, then update the target waypoint and send the carrot chasing waypoint
