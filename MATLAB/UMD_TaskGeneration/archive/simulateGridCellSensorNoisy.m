function [cellsInView, signals, cellStateMat, cellMsmtMat ] = simulateGridCellSensorNoisy( xcp, ycp, Rsense, x, N, cellStateMat, cellMsmtMat, numNodesMat, communicationTopology, mG, nG )
% Input:
        ...
        for by = [binYStart:1:binYEnd]
            ...
            if ( norm(controlPt-agent) <= Rsense )
                ...
                if ( cellStateMat(by,bx) == 1 ) % is a node
                    signals(k) = quantizedSensor(mG, nG, 1);
                else
                    signals(k) = quantizedSensor(mG, nG, 0);
                end
                k = k + 1;
            end
        end
