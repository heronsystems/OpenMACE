function updateWpts( ROS_MACE, wptsXY, wptIndex) 

persistent sentWptIndex
if isempty(sentWptIndex)
    sentWptIndex = zeros(size(wptIndex));
end

for i = 1:1:ROS_MACE.N
    if  sentWptIndex(i) ~= wptIndex(i)
        sentWptIndex(i) = wptIndex(i);
