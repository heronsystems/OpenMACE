...
numAvailableAgents = 2; %maximum number of vehicles going to be allowed to participate
...
numInitialTasks = 9; %number of initial tasks to seed the environment with
startIndex = 1;

for assignmentRound = 1:2
    
	availableTasks = ADCA_AvailableTaskQueue();

    
	for m = (startIndex):(numInitialTasks*assignmentRound)
    ...
    %update the startIndex to be correct
    startIndex = startIndex + numInitialTasks;
